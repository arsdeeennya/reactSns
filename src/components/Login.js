...
const initialState = {
  // ログインしてる時はtrue、ログイン終えるとfalse
  isLoading: false,
  // login or register 
  isLoginView: true,
  error: "",
  //views.obtain_auth_tokenは
  // usernameとpasswordというキーワードを用いてtokenを返してくれるから
  credentialsLog: {
    username: "",
    password: "",
  },
...
const Login = (props) => {
  ...
  const inputChangedReg = () => (event) => {
    ...
    //cred[event.target.name] = event.target.value;ここでstateに格納している、からのdispatchのpayloadでreducerに送っている
    dispatch({
      type: INPUT_EDIT_REG,
  ...
  const login = async (event) => {
    // submitを押した時にpageがrefleshするのを防ぐため
